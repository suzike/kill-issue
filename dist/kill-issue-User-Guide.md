...
```bash
# 建通用上下文目录 000_Context/（模型/代码/需求/接口/标定/测试/日志）+ 台账 问题清单.md
...
# 建问题文件夹（含问题描述模板 + 附件/ + 证据/）
python scripts/init_context.py <项目根> --issue "001-问题精确描述"
```

### 3. 工程师输入方式（不需要填任何表单）

- 把问题文件丢进来：logs、CANoe/CANape 录制的报文（.asc/.blf/.mf4/.dat）、截图、trace，放入问题文件夹 `附件/` 或直接拖入对话。
- 在输入框用自然语言描述问题，一句话即可（如"XX 模式下某执行器开度很小调不了"）。
- 其余全部交给 agent：它会自动提取信息补全 `问题描述.md`（现象/预期/复现条件/环境版本/附件清单），缺的信息会在开始排查前向你澄清确认。

### 4. 项目目录约定（编号体系）

```
<项目根>/
├── 000_Context/          # 通用上下文（所有问题共享）：01_Model 02_Code 03_Requirement
│                         #   04_Interface(接口表/ARXML/DBC) 05_Calibration 06_Test 07_Logs
├── 001-问题精确描述/      # 问题 001：问题描述.md + 附件/(该问题专属 logs/报文) + 证据/ + 报告
├── 002-问题精确描述/      # 问题 002
...
- 通用上下文文件命名不规整时，agent 会主动按类别归类到 000_Context 子目录。上下文材料**分级**：模型、代码必须提供；问题附件含 log/报文时必须有 DBC（否则报文解析不了）；需求、标定、测试说明等按问题需要提供，**不需要 7 类给全**。缺必须项时 agent 会提醒补充。
- 问题附件（CANoe .asc/.blf、CANape .mf4/.dat 等）是**该问题的专属上下文**，放对应问题文件夹 `附件/`，不进通用上下文。
- 问题串行处理：001 弄完、登记完，再开 002。
- 上下文材料互相冲突时，**以模型和代码为准**（模型 vs 生成代码不一致时以生成代码为准）。

### 5. 排查与报告

调用后 agent 会按流程执行：读上下文 → 澄清提问（现象/预期、复现条件、版本、附件、报告形式）→ 判定归属层 → 定位证据（代码块带注释、模型子系统截图、需求/接口比对、日志时间线）→ 按所选模板出报告。
